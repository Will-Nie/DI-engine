...
from ding.policy.trex import TREX

@POLICY_REGISTRY.register('trex_ppo_offpolicy')
class TREXPPOOffPolicy(Policy):
    r"""
    Overview:
    ...
    def _process_transition(self, obs: Any, model_output: dict, timestep: namedtuple) -> dict:
        ...
        """
        if os.path.exists('/home/SENSETIME/nieyunpeng/DI-engine/ding/policy/reward_net.params'):
            pass
        else:
            Agent = TREX('pong') #specifically for pong for now
            Agent.main()
        reward_model = TREX_Model()
        reward_model.load_state_dict(torch.load('/home/SENSETIME/nieyunpeng/DI-engine/ding/policy/reward_net.params'))
        to_obs = torch.tensor(obs).unsqueeze(0)
        #print(to_obs.shape)
        #print(to_obs)
        new_reward,_=reward_model.reward_forward(to_obs)
        new_reward = new_reward.unsqueeze(0).detach().numpy()
        #print(new_reward)
