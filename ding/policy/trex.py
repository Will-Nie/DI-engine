...
from ding.model.template.trex import TREX_Model
...
class TREX(object):
    ...
    def __init__(self, env_name, reward_model_path = './reward_net.params', models_dir = '.', num_trajs = 0, num_snippets = 6000, seed = 0):
    ...
    def generate_novice_demos(self, env, model_dir):
    ...
    def create_training_data(self, demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length):
    ...
    # Train the network
    def learn_reward(self, reward_network, optimizer, training_inputs, training_outputs, num_iter, l1_reg, checkpoint_dir):
    ...
    def main(self):
        ...
        demonstrations, learning_returns, learning_rewards = self.generate_novice_demos(demon_env, self.models_dir)
        ...
        training_obs, training_labels = self.create_training_data(demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length)
        print("num training_obs", len(training_obs))
        print("num_labels", len(training_labels))
    
        # Now we create a reward network and optimize it using the training data.
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        reward_net = TREX_Model()
        reward_net.to(device)
        import torch.optim as optim
        optimizer = optim.Adam(reward_net.parameters(),  lr=lr, weight_decay=weight_decay)
        self.learn_reward(reward_net, optimizer, training_obs, training_labels, num_iter, l1_reg, self.reward_model_path)
        #save reward network
        torch.save(reward_net.state_dict(), self.reward_model_path)
        
        #print out predicted cumulative returns and actual returns
        with torch.no_grad():
            pred_returns = [self.predict_traj_return(reward_net, traj) for traj in demonstrations]
        for i, p in enumerate(pred_returns):
            print(i,p,sorted_returns[i])

        print("accuracy", self.calc_accuracy(reward_net, training_obs, training_labels))
